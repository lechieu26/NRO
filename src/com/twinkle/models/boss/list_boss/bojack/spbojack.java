...
public class spbojack extends Boss {

    public spbojack() throws Exception {
        super(BossID.NKHONG, BossesData.NKHONG);
    }
    ...
    public void reward(Player plKill) {
        ...
        if (Util.isTrue(100, 100)) {
            if (Util.isTrue(50, 50)) {
                Service.gI().dropItemMap(this.zone, Util.ratiItem(zone, 674, 3, this.location.x, this.location.y, plKill.id));
                return;
            }
            Service.gI().dropItemMap(this.zone, Util.ratiItem(zone, itemDos[randomDo], 3, this.location.x, this.location.y, plKill.id));
