    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask,0, data_card, bill_data,data_item_time_sieu_cap,itemBanh);
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = " update player set data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + "data_black_ball = ?, data_side_task = ?, violate=?, data_charm = ?, skills = ?,"
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =?,data_banh =? where id = ?";
