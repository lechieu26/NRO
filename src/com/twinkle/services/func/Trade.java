...
import com.twinkle.jdbc.daos.HistoryTransactionDAO;
...
//        System.out.println("quantity: " + quantity);
        ...
        if (true) {
           ...
           this.cancelTrade();
            Service.gI().sendThongBaoFromAdmin(pl,
                  "|5|Vui Lòng Kích Hoạt Thành Viên Để Mở Khoá Chức Năng Giao Dịch!");
    ...
    private void removeItemTrade(Player pl, byte index) {
        ...
        try {
            ...
            pl.sendMessage(msg);
            msg.cleanup();
            Service.gI().sendThongBao(pl, "|5|Vui Lòng Kích Hoạt Thành Viên Để Mở Khoá Chức Năng Giao Dịch");
        } catch (Exception e) {
        }
    }
    private void removeItemnottran(Player pl, byte index) {
        Message msg;
        try {
            msg = new Message(-86);
            msg.writer().writeByte(2);
            ...
            pl.sendMessage(msg);
            msg.cleanup();
            Service.gI().sendThongBao(pl, "Vật phẩm không thể giao dịch");
        } catch (Exception e) {
        }
    }
    ...
    private boolean isItemCannotTran(Item item) {
        ...
        switch (item.template.type) {
            case 27: //
                if (item.template.id != 457 && item.template.id == 590 || item.template.id == 2000 || item.template.id == 2001 || item.template.id == 2002) {
                    return true;
                } else {
                    return false;
                }
