...
import com.twinkle.utils.FileIO;
...
import com.twinkle.models.matches.PVPManager;
import com.twinkle.models.matches.TOP;
import com.twinkle.models.player.Player;
import com.twinkle.models.shop.ItemShop;
...
import static com.twinkle.services.func.SummonDragon.DRAGON_SHENRON;
...
//                }
            ...
            if (text.equals("adm")) {
                ...
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "|2|NRO Twinkle\nSố người online: " + songuoi + "\n"
           ...
           /* if (player.nPoint.power > player.nPoint.getPowerLimit()) {
            ...
            switch (type) {
                ...
                case 2:
                    ...
                    player.nPoint.tiemNangUp(param);
                    if (pet != null) {
                        pet.nPoint.powerUp(param * 200);
                        pet.nPoint.tiemNangUp(param);
                    }
                    break;
                default:
                    player.nPoint.powerUp(param);
...
//            Send_Info_NV(pl);
    ...
    public void doiTVSangRuby(Player player, String soluong) {
     ...
     public void doiRubySangPet(Player player, String soluong) {
        try {
            int sl = Integer.parseInt(soluong);
            int tongso = sl*20000;
            if(sl <=0 || sl>=9999){
                Service.gI().sendThongBao(player, "Nhập lại đi ku , mài nhập cái ngôn ngữ choá nó còn không hiểu");
                return;
            }
            Item thoivang = ItemService.gI().createNewItem((short) 2028, sl);
            if (player.inventory.ruby < tongso) {
                Service.getInstance().sendThongBao(player, "Không đủ ruby để đổi: " + sl + " hộp pet");
                return;
            }
            player.inventory.ruby -= tongso;
            Service.getInstance().sendMoney(player);
            InventoryServiceNew.gI().addItemBag(player, thoivang);
            InventoryServiceNew.gI().sendItemBags(player);
            Service.getInstance().sendThongBao(player, "Bạn đã nhận: " + sl + " hộp pet");
        } catch (Exception e) {
        }
    }
      public void doiRubySangLinhthu(Player player, String soluong) {
        try {
            int sl = Integer.parseInt(soluong);
            int tongso = sl*20000;
            if(sl <=0 || sl >=9999){
                Service.gI().sendThongBao(player, "Nhập sai giá trị cái đầu moi");
                return;
            }
            Item thoivang = ItemService.gI().createNewItem((short) 2087, sl);
            if (player.inventory.ruby < tongso) {
                Service.getInstance().sendThongBao(player, "Không đủ ruby để đổi: " + sl + " hộp pet");
                return;
            }
