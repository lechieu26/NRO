...
import com.twinkle.services.ChatGlobalService;
...
import com.twinkle.services.SubMenuService;
import com.twinkle.services.FlagBagService;
...
import com.twinkle.services.func.UseItem;
...
import static com.twinkle.services.func.Input.CHOOSE_LEVEL_BDKB;
import static com.twinkle.services.func.Input.NUMERIC;
import com.twinkle.services.func.LuckyRound;
import com.twinkle.services.func.SummonDragon;
import com.twinkle.services.func.TransactionService;
...
public class Controller implements IMessageHandler {
                                ...
                                if (card != null) {
                                    ...
                                    if (card.Used == 0) {
                                        ...
                                        RadarCard radarTemplate = RadarService.gI().RADAR_TEMPLATE.stream()
                                                .filter(r -> r.Id == idC).findFirst().orElse(null);
    ...
    public void login2(MySession session, Message msg) {
        ...
        // Service.gI().sendThongBaoOK(session, "Vui lòng đăng ký tài khoản tại trang
        // chủ!");
    }
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Chú Bé Crazy\n"
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "NRO Twinkle\nxin chào các bạn !!");
    }

    private void clearVTSK(Player player) {
        player.inventory.itemsBag.stream().filter(item -> item.isNotNullItem() && item.template.id == 610)
                .forEach(item -> {
                    InventoryServiceNew.gI().subQuantityItemsBag(player, item, item.quantity);
                });
        player.inventory.itemsBox.stream().filter(item -> item.isNotNullItem() && item.template.id == 610)
                .forEach(item -> {
